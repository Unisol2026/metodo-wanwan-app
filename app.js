...
function renderTrail(axisId){
  ...
  chs.slice().reverse().forEach((ch,ri)=>{
    ...
    const stop=el('div','tm-stop '+st.state+' '+(idx%2===0?'r':'l'));
    ...
    if(st.state!=='locked')node.onclick=()=>renderChapter(axisId,ch.id);
